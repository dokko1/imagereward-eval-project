...
from ImageReward.utils import load
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            'Whether training should be resumed from a previous checkpoint. Use a path or'
    ...
    # For evaluation
    parser.add_argument(
        "--eval_num_imgs_to_generate",
        type=int,
        default=1,
        help="Number of images to generate per prompt during `eval`. Default to 1."
    )
    parser.add_argument(
        "--id_column",
        type=str,
        default="id",
        help="The column of the dataset containing IDs of samples. Default to None, as there isn't.",
    )
    parser.add_argument(
        "--run_name",
        type=str,
        default="text2image-refl",
        help="The project name used by the accelerator's tracker."
    )
...
class Trainer(object):
    def __init__(self, pretrained_model_name_or_path, train_data_dir, args) -> None:
        ...
        self.reward_model = load(args.image_reward_version, 
                                 device=self.accelerator.device, 
                                 med_config="train/src/config/med_config.json", 
        ...
        def save_model_hook(models, weights, output_dir):
            if self.accelerator.is_main_process:
                ...
                for model in models:
                    if isinstance(self._unwrap_model(model), type(self._unwrap_model(self.unet))):
        ...
        def load_model_hook(models, input_dir):
            ...
            while len(models) > 0:
                ...
                if isinstance(model, type(self._unwrap_model(self.unet))):
                    unet_ = model
                else:
                    raise ValueError(f"unexpected save model: {model.__class__}")

            lora_state_dict, _ = StableDiffusionXLLoraLoaderMixin.lora_state_dict(input_dir)
        ...
        if args.id_column is None:
            id_column = dataset_columns[2] if dataset_columns is not None else column_names[2]
        else:
            id_column = args.id_column
            if id_column not in column_names:
                raise ValueError(
                    f"--id_column' value '{args.id_column}' needs to be one of: {', '.join(column_names)}"
            )
        ...
        def collate_fn(examples):
            ...
            ids = [example[id_column] for example in examples]
        ...
        if self.accelerator.is_main_process:
            ...
            # Delete 'validation_prompts' attribute as trackers cannot store list-valued variables
            delattr(cp_args, "validation_prompts")
            self.accelerator.init_trackers("text2image_refl_sdxl_lora", config=vars(cp_args))
            self.accelerator.trackers[0].run.name = args.run_name
            del cp_args
    ...
    def _log_validation(self, args, global_step):
        # create pipeline
        ...
        for i in range(len(args.validation_prompts)):
            pipeline_args = {"prompt": args.validation_prompts[i]}
            ...
            with autocast_ctx:
                image = pipeline(**pipeline_args, num_inference_steps=100, generator=generator).images[0]
                    ...
                    if self.accelerator.is_main_process:
                        ...
                        if global_step % args.checkpointing_steps == 0:
                            ...
                            if args.validation_prompts is not None:
                                ...
                                self._log_validation(args, global_step)
